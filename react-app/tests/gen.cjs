...
var _jspdfMod = require("jspdf"); var import_jspdf = { default: _jspdfMod.jsPDF || _jspdfMod.default || _jspdfMod };
...
function drawElev(d, data, wall, ox, oy, mW, mH) {
  ...
  for (const dr of doors) {
    ...
    d.setFont("helvetica", "normal");
    d.setTextColor(...C.DARK);
    d.text(`${dr.width}'\xD7${clampedH}'`, dX + dW / 2, dY + dH2 + 3, { align: "center" });
    if (dr.type === "rollUp") {
...
function drawS4(d, data) {
  ...
  if (a.gutters) {
    ...
    d.setLineWidth(0.8);
    d.line(rX, rY, rX, rY + rL);
    d.line(rX + rW, rY, rX + rW, rY + rL);
    d.setFontSize(4);
    d.setFont("helvetica", "bold");
    d.setTextColor(...C.TEAL);
    ...
    const dsSize = 2;
    d.setFillColor(...C.TEAL);
    d.rect(rX - dsSize + 0.5, rY - 0.5, dsSize, dsSize, "F");
    d.rect(rX - dsSize + 0.5, rY + rL - dsSize + 0.5, dsSize, dsSize, "F");
    d.rect(rX + rW - 0.5, rY - 0.5, dsSize, dsSize, "F");
    d.rect(rX + rW - 0.5, rY + rL - dsSize + 0.5, dsSize, dsSize, "F");
    d.setFontSize(3.5);
    d.setTextColor(...C.TEAL);
...
function generateStandaloneConstructionPlan(data) {
  const doc = new import_jspdf.default({ orientation: "portrait", unit: "mm", format: "letter" });
  drawS1(doc, data);
  doc.addPage();
  drawS2(doc, data);
  doc.addPage();
